...
def setup_db(tables):
    ...
    for tbl in tables:
        try:
            ...
            # r.db(MY_DB).table(tbl).index_create("auth",[r.row["email"],r.row["password"]]).run(connection)
            # r.db(MY_DB).table(tbl).index_wait("auth").run(connection)
